    ...
    def train_update(self, inst):
        if self.method == 'hlr':
            ...
            for (k, x_k) in inst.fv:
                rate = (1. /
                        (1 + inst.p)) * self.lrate / math.sqrt(1 +
                                                               self.fcounts[k])
                ...
                self.weights[
                    k] -= rate * self.l2wt * self.weights[k] / self.sigma**2
            ...
            for inst in testset:
                ...
                for i in range(inst.right):
                    f.write('1.0\t%.4f\t%.4f\t%.4f\t%s\t%s\t%d\t%s\n' %
                            (pp, inst.h, hh, inst.lang, inst.uid, inst.ts,
                             inst.lexeme))
...
def read_data(input_file,
    ...
    for i, row in enumerate(reader):
        ...
        if not omit_lexemes:
            fv.append(
                (intern('%s:%s' % (row['learning_language'], lexeme_string)),
                 1.))
        instances.append(
...
if __name__ == "__main__":
    ...
    # read data set
    trainset, testset = read_data(args.input_file, args.method, args.b, args.l,
                                  args.max_lines)
